  ...
  async parseCV(input: CVInput): Promise<ParsedCV> {
    ...
    if (input.vacancyText && input.vacancyText.trim()) {
      if (input.finalGradeMode) {
        // Final-grade mode: only do tag alignment + match score, NO bullet rewriting (avoid conflict)
        focusBlock = `\n\n--- VACATURE-CONTEXT (Punt 7, beperkt vanwege final-grade modus) ---\nEr is een vacaturetekst meegegeven, maar final-grade modus staat aan — dus GEEN bullet-rewriting. Alleen:\n\n1. TAGS: De 5 tags ("WAAR DEZE PROFESSIONAL STERK IN IS") aansluiten op de kerneisen van de vacature.\n\n2. MATCH-SCORE — vul analysis.vacancyMatches met eerlijke scores:\n   - { "title": "Algehele match", "score": 0-100 }\n   - { "title": "Werkervaring relevantie", "score": 0-100 }\n   - { "title": "Sectorkennis", "score": 0-100 }\n\nVACATURETEKST:\n${input.vacancyText.trim()}`;
      } else {
